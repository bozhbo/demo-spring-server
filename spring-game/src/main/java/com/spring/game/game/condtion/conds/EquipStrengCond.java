...
public class EquipStrengCond extends AbstractConditionCheck {
	private int checkLv;
	private int equipNum;
	
	public EquipStrengCond() {
		
	}
	
	public EquipStrengCond(int checkLv, int equipNum) {
	...
	public AbstractConditionCheck generate(String[] agrs) {
		...
		return new EquipStrengCond(checkLv, equipNum);
	}
	...
	public int checkCond(RoleInfo roleInfo, int action,
		...
		if (questInProgress == null 
				|| (action != 0 && action != ActionType.action181.getType() && action != ActionType.action77.getType())) {
			return 0;
		}
		...
		if (roleLoadInfo.getBagEquipMap() != null && !roleLoadInfo.getBagEquipMap().isEmpty()) {
			for (EquipInfo equipInfo : roleLoadInfo.getBagEquipMap().values()) {
				if (equipInfo.getLevel() >= checkLv) {
					flag++;
				}
				
				if (equipInfo.getLevel() > maxVal) {
					maxVal = equipInfo.getLevel();
				}
			}
		}
