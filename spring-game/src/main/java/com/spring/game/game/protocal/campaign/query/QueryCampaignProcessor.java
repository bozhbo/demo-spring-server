...
public class QueryCampaignProcessor extends ProtocolProcessor {
	...
	public void execute(Request request, Response response) {
		...
		header.setMsgType(Command.QUERY_CAMPAIGN_RESP);
		int roleId = header.getUserID0();
		QueryCampaignResp resp = campaignMgtService.queryCampaign(roleId);
		message.setHeader(header);
		message.setBody(resp);
		response.write(message);

		if (logger.isInfoEnabled()) {
			logger.info(Resource.getMessage("game", "GAME_CAMPAIGN_INFO_1") + ": result=" + resp.getResult() + ",roleId="
					+ roleId);
		}
	}
