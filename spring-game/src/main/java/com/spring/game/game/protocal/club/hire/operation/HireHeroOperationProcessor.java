...
public class HireHeroOperationProcessor extends ProtocolProcessor{
	...
	public void execute(Request request, Response response) {
		...
		header.setMsgType(Command.HIRE_HERO_RESP);
		
		int roleId = header.getUserID0();
		
		HireHeroOperationReq req = (HireHeroOperationReq) msg.getBody();
		
		HireHeroOperationResp resp = hireHeroMgtService.operateHireHero(roleId, req);
		...
		if(logger.isInfoEnabled()){
			logger.info(Resource.getMessage("game", "GAME_ROLE_CLUB_INFO_16") + ": result=" + resp.getResult() + ",roleId="
					+ roleId);
		}
