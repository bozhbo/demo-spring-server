...
public class ClubUpgradeProcessor extends ProtocolProcessor{
	...
	public void execute(Request request, Response response) {
		...
		header.setMsgType(Command.CLUB_UP_RESP);
		
		ClubUpgradeReq req = (ClubUpgradeReq) msg.getBody();
		
		ClubUpgradeResp resp = clubMgtService.clubUpgrade(roleId, req);
		...
		if(logger.isInfoEnabled()){
			logger.info(Resource.getMessage("game", "GAME_ROLE_CLUB_INFO_10") + ": result=" + resp.getResult() + ",roleId="
					+ roleId);
		}
