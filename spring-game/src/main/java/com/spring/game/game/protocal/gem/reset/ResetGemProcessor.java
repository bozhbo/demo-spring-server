...
public class ResetGemProcessor extends ProtocolProcessor {
	...
	public void execute(Request request, Response response) {
		...
		header.setMsgType(Command.RESET_GEM_RESP);
		int roleId = header.getUserID0();
		ResetGemResp resp = gemMgtService.resetGem(roleId);
		message.setHeader(header);
		message.setBody(resp);
		response.write(message);

		if (logger.isInfoEnabled()) {
			logger.info(Resource.getMessage("game", "GAME_GEM_INFO_2") + ": result=" + resp.getResult() + ",roleId="
					+ roleId);
		}
	}
