...
public class MergeHeroProcessor extends ProtocolProcessor {
	...
	public void execute(Request request, Response response) {
		...
		MergeHeroReq req = (MergeHeroReq) message.getBody();
		head.setMsgType(Command.MERGE_HERO_RESP);
		int roleId = head.getUserID0();
		MergeHeroResp resp = heroMgtService.mergeHero(roleId, req);
		...
		if (logger.isInfoEnabled()) {
			logger.info(Resource.getMessage("game", "GAME_HERO_INFO_2") + ":result=" + resp.getResult() + ",roleId="
					+ roleId);
		}
	}
}
