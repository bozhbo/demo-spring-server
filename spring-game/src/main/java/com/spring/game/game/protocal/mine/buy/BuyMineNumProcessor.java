...
public class BuyMineNumProcessor extends ProtocolProcessor {
	...
	public void execute(Request request, Response response) {
		...
		header.setMsgType(Command.BUY_MINE_NUM_RESP);
		int roleId = header.getUserID0();
		BuyMineNumResp resp = mineMgtService.buyMineNum(roleId);
		message.setHeader(header);
		message.setBody(resp);
		response.write(message);

		if (logger.isInfoEnabled()) {
			logger.info(Resource.getMessage("game", "GAME_MINE_INFO_12") + ": result=" + resp.getResult() + ",roleId="
					+ roleId );
		}
	}
