...
public class QueryMineProcessor extends ProtocolProcessor {
	...
	public void execute(Request request, Response response) {
		...
		header.setMsgType(Command.QUERY_MINE_RESP);
		int roleId = header.getUserID0();
		QueryMineResp resp = mineMgtService.queryMine(roleId);
		message.setHeader(header);
		message.setBody(resp);
		response.write(message);

		if (logger.isInfoEnabled()) {
			logger.info(Resource.getMessage("game", "GAME_MINE_INFO_1") + ": result=" + resp.getResult() + ",roleId="
