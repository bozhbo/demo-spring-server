 ...
 *
 */
public class QuerySevenDayProcessor extends ProtocolProcessor {
	...
	public void execute(Request request, Response response) {
		...
		header.setMsgType(Command.QUERY_SEVEN_DAY_RESP);
		int roleId = header.getUserID0();
		QuerySevenDayResp resp = opActivityMgrService.querySevenDay(roleId);
		message.setHeader(header);
		message.setBody(resp);
		response.write(message);

		if (logger.isInfoEnabled()) {
			logger.info(Resource.getMessage("game", "GAME_OPACTIVITY_INFO_9") + ": result=" + resp.getResult() + ",roleId="
					+ roleId + ",curDay=" + resp.getCurDay());
		}
	}
