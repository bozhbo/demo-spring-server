...
public class OneKeyEnergyProcessor extends ProtocolProcessor{
	...
	public void execute(Request request, Response response) {
		...
		header.setMsgType(Command.ONE_KEY_ENERGY_RESP);
		
		int roleId = header.getUserID0();

		OneKeyEnergyReq req = (OneKeyEnergyReq) msg.getBody();
		
		OneKeyEnergyResp resp = roleRelationMgtService.oneKeyEnergy(roleId, req);
		...
		if(logger.isInfoEnabled()){
			logger.info(Resource.getMessage("game", "GAME_ROLE_RELATION_INFO_9") + ": result=" + resp.getResult() + ",roleId="
					+ roleId);
		}
	}
}
