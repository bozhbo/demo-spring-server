 ...
 *
 */
public class MapPVPFightProcessor extends ProtocolProcessor {
	...
	public void execute(Request request, Response response) {
		...
		MapPvpFightReq req = (MapPvpFightReq) message.getBody();
		MapPvpFightResp resp = sceneMgtService.mapPvpFightStart(roleId,req);
		if(resp.getResult() != 1)
		{
		...
		if (logger.isInfoEnabled()) {
			logger.info(Resource.getMessage("game", "GAME_SCENE_INFO_8")+",roleId="+roleId+",beRoleId="+req.getBeRoleId()+",result"+resp.getResult());
		}
	}
