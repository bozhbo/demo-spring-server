 ...
 * @author nijy
 */
public class UpdateShowPlanProcessor extends ProtocolProcessor {
	...
	public void execute(Request request, Response response) {
		...
		UpdateShowPlanReq req = (UpdateShowPlanReq) message.getBody();
		int roleId = head.getUserID0();
		UpdateShowPlanResp resp = shizhuangMgtService.updateShopPlan(roleId, req);
		head.setMsgType(Command.SHIZHUANG_PLAN_RESP);
		message.setBody(resp);
		response.write(message);
		if (logger.isInfoEnabled()) {
			logger.info(Resource.getMessage("game", "GAME_SHIZHUANG_INFO_1") + ": result = " + resp.getResult()
					+ ",roleId=" + roleId);
		}
