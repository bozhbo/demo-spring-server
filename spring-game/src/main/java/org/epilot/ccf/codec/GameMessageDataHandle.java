...
public class GameMessageDataHandle extends ProtocolChange {
				...
				while(iterator.hasNext())
				{
					...
					else if(MethodType[i].startsWith("classArray-"))//对象数组类型
					{
						...
						if(count!=null)
						{
							...
							{
								if (num > 200) {
									throw new Exception(obj.getClass().getName() + " receive num is " + num);
								}
					...
					else
					{
						...
						method.invoke(obj, new Object[]{ReadBuffer(MethodType[i],MethodCode[i],
								stringHandle, byteBufferDataHandle)});
					}
					i++;
				}
			}
				...
				while(iterator.hasNext())
				{
					...
					else
					{
						...
						WriteBuffer(MethodType[i],MethodCode[i], stringHandle,
								method.invoke(obj),byteBufferDataHandle);
					}
					i++;
				}
			}
			...
			ByteBufferDataHandle byteBufferDataHandle)
	{
		if(endian.equals("0"))
		{
			...
			{
				byte b [] = null;
				int i = byteBufferDataHandle.getBigInt();
				if(i>0)
				{
					b = new byte[i];
		...
		else if(endian.equals("1"))
		{
			...
			{
				byte b [] = null;
				int i = byteBufferDataHandle.getLittleInt();
				if(i>0)
				{
					b = new byte[i];
	...
	private void WriteBuffer(String type,String endian,AbstractStringHandle stringHandle,
			Object obj,ByteBufferDataHandle byteBufferDataHandle)
	{
		if(endian.equals("0"))
		{
			if(type.equalsIgnoreCase("byte"))
			...
			else if(type.startsWith("class [B"))//byte数组对象
			{
				if(obj!=null)
				{
					byteBufferDataHandle.setBigData(((byte[])obj).length);
		...
		else if(endian.equals("1"))
		{
			...
			else if(type.startsWith("class [B"))//byte数组对象
			{
				if(obj!=null)
				{
					byteBufferDataHandle.setLittleData(((byte[])obj).length);
